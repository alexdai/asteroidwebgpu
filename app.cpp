...
char const Asteroids::blockrender_vert_wgsl[] = R"(
	struct VertexIn {
...
char const Asteroids::blockrender_frag_wgsl[] = R"(
	fn saturate(x: f32) -> f32 {
		return clamp(x, 0.0, 1.0);
	}
	@stage(fragment)
...
WGPUShaderModule Asteroids::createShader(const char* const code, const char* label = nullptr) {
...
WGPUBuffer Asteroids::createBuffer(const void* data, size_t size, WGPUBufferUsage usage) {
...
void Asteroids::createPipelineAndBuffers() {
    // compile shaders
...
Asteroids::Asteroids(AsteroidsSimulation* asteroids£¬ unsigned int minCmdLsts)
...
void Asteroids::CreateMeshes()
{
    ...
    UINT64 totalSize = asteroidIBOffset + asteroidIBSize;
